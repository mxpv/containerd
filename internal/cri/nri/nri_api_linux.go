...
func (a *API) nriPodSandbox(pod *sstore.Sandbox) *criPodSandbox {
	...
	criPod.pid = pod.Instance.Pid

	if err := typeurl.UnmarshalTo(pod.Instance.Spec, &criPod.spec); err != nil {
		log.L.WithError(err).Errorf("failed to unmarshal spec for sandbox %s", pod.ID)
	}
...
func (p *criPodSandbox) GetLabels() map[string]string {
	...
	for key, value := range p.Instance.Labels {
		labels[key] = value
	}
