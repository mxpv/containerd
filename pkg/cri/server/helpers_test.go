...
func TestLocalResolve(t *testing.T) {
	image := images.Image{
		...
		Labels: map[string]string{
			containerd.ImageLabelConfigDigest: "sha256:c75bebcdd211f41b3a460c7bf82970ed6c75acaab9cd4c9a4e125b03ca113799",
			imageLabelChainID:                 "test-chain-id-1",
			imageLabelRepoTag:                 "docker.io/library/busybox:latest",
			imageLabelRepoDigest:              "docker.io/library/busybox@sha256:e6693c20186f837fc393390135d8a598a96a833917917789d63766cab6c59582",
			imageLabelSize:                    "10",
		},
	...
	for _, ref := range []string{
		...
		// Truncated indices
		"sha256:e6693c20186f837fc393390135d8a598a96a833917917789d63766cab6c59582",
		"sha256:e6693c20186f837fc393390135d8a",
	} {
		t.Run(ref, func(t *testing.T) {
			img, err := c.findImage(ctx, ref)
			assert.NoError(t, err)
			assert.Equal(t, image, img)
		})
	}

	img, err := c.findImage(ctx, "randomid")
	assert.Equal(t, errdefs.IsNotFound(err), true)
	assert.Equal(t, images.Image{}, img)
}
